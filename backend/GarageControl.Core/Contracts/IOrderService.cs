...
namespace GarageControl.Core.Contracts
{
        public interface IOrderService
    {
        ...
        Task<MethodResponseVM> CreateOrderAsync(string userId, string workshopId, CreateOrderVM model);
        Task<OrderDetailsVM?> GetOrderByIdAsync(string id, string workshopId);
        Task<OrderInvoiceVM?> GetOrderInvoiceByIdAsync(string id);
        Task<MethodResponseVM> UpdateOrderAsync(string userId, string id, string workshopId, UpdateOrderVM model);
