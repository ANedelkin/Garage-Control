    ...
    public class ModelService : IModelService
    {
        ...
        public async Task CreateModel(ModelVM model, string userId)
        {
            ...
            if (workshopId != null)
            {
                ...
                await _activityLogService.LogActionAsync(userId, workshopId, $"added model <a href='/makes-and-models/{model.MakeId}/model/{carModel.Id}?highlight=true' class='log-link target-link'>{carModel.Name}</a> to make <a href='/makes-and-models/{model.MakeId}?highlight=true' class='log-link target-link'>{make?.Name ?? "Unknown"}</a>");
            }
        }
        ...
        public async Task UpdateModel(string id, ModelVM model, string userId)
        {
            ...
            if (carModel != null)
            {
                ...
                {
                    await _activityLogService.LogActionAsync(userId, workshopId, $"renamed model <b>{oldName}</b> to <a href='/makes-and-models/{carModel.CarMakeId}/model/{carModel.Id}?highlight=true' class='log-link target-link'>{model.Name}</a> (Make: <a href='/makes-and-models/{carModel.CarMakeId}?highlight=true' class='log-link target-link'>{carModel.CarMake.Name}</a>)");
                }
            }
        }
        ...
        public async Task MergeModelWithGlobal(string customModelId, string globalModelId, string userId)
        {
            ...
            if (workshopId != null)
            {
                await _activityLogService.LogActionAsync(userId, workshopId, $"merged custom model <b>{customModelName}</b> into <a href='/makes-and-models/{globalModel.CarMakeId}/model/{globalModelId}?highlight=true' class='log-link target-link'>{globalModelName}</a>");
            }
        }
